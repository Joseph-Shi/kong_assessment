describe('create service and route', () => {
  it('open workspace', () => {
    ...
    cy.get('[data-testid="gateway-service-url-input"]').type('https://localhost:8002/test');
    cy.get('[data-testid="gateway-service-name-input"]').clear('n');
    cy.get('[data-testid="gateway-service-name-input"]').type('new-service-for-testing');
    cy.get('[data-testid="service-create-form-submit"]').click();
    cy.get('.alert-message > .k-button').click();
    /* ==== Generate route ==== */
    cy.get('[data-testid="route-form-name"]').type('route_1');
    ...
    cy.get('[data-testid="confirmation-input"]').type('route_1');
    ...
    cy.get('[data-testid="confirmation-input"]').clear('new-service-for-testing');
    cy.get('[data-testid="confirmation-input"]').type('new-service-for-testing');
    cy.get('[data-testid="modal-action-button"]').click();
  })
})